...
export default async function ReceiptPage(props: { params: { id: string } }) {
      ...
      <Container>
        <Card title="Receipt" subtitle="Could not load purchase intent.">
          <pre style={{ whiteSpace: "pre-wrap" }}>
            {JSON.stringify(
              {
                message: "Invalid receipt id in URL.",
                received: id ?? null,
  ...
  const { data: intent, error } = await supabase
    ...
    .select(
      "id,status,tranche_id,price_usdc_used,tokens_skopi,amount_usdc_atomic,reference_pubkey,created_at,confirmed_at,tx_signature,failure_reason,updated_at"
    )
    .eq("id", id)
    .single();
