...
export async function POST(req: NextRequest) {
  try {
    ...
    // ---- Affiliate payout chain (3 levels) ----
    const ft_ref_code = body.refCode ?? null;

    let lt_ref_code: string | null = null;
    let tt_ref_code: string | null = null;

    if (ft_ref_code) {
      const { data: mp1 } = await supabase
        .from("marketing_partners")
        .select("parent_ref_code")
        .eq("referral_code", ft_ref_code)
        .maybeSingle();

      lt_ref_code = (mp1?.parent_ref_code ?? null) as string | null;

      if (lt_ref_code) {
        const { data: mp2 } = await supabase
          .from("marketing_partners")
          .select("parent_ref_code")
          .eq("referral_code", lt_ref_code)
          .maybeSingle();

        tt_ref_code = (mp2?.parent_ref_code ?? null) as string | null;
      }
    }
    ...
    const insertPayload = {
      ...
      // Store payout chain (3 levels deep)
      ft_ref_code,
      lt_ref_code,
      tt_ref_code,

      // Keep the raw last-touch ref code too (marketing attribution),
      // if you want it later for analytics:
      // lt_ref_code: body.lastTouch?.refCode ?? null,  <-- don't use; it would overwrite payout chain
    };
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, status, amount_usdc_atomic, treasury_address, usdc_mint, reference_pubkey, ft_ref_code, lt_ref_code, tt_ref_code, created_at"
      )
      .single();
