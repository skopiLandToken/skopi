...
export default async function ReceiptPage(props: {
  searchParams?: any; // can be object OR Promise in some Next builds
}) {
  // Normalize searchParams (handles Promise vs object)
      ...
      <Container>
        <Card title="Receipt" subtitle="Could not load purchase intent.">
          <pre style={{ whiteSpace: "pre-wrap" }}>
            {JSON.stringify(
              {
                message: "Missing or invalid receipt id. Use /receipt?id=<uuid>.",
                received: id || null,
                debug_searchParams: sp ?? null,
              },
              null,
              2
            )}
